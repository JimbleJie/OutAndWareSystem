...
<%
	...
	String[] supname = request.getParameterValues("supname");
	String[] supunityname = request.getParameterValues("supunityname");
	String[] suptel = request.getParameterValues("suptel");
	DBUtils db = new DBUtils();
	Connection cc = db.getConnection();
	Statement cs = cc.createStatement();
	SupplierController spc = new SupplierController();
	if (supname != null) {
		for (int i = 0; i < supname.length; i++) {
			spc.InsertSupplier(supname[i], supunityname[i], suptel[i], company);
		}
	}
	List<Supplier> srl = new ArrayList<Supplier>();
	ResultSet rs = cs
			.executeQuery("select supname,supunityname,suptel from supinfo where username='" + company + "'");
	while (rs.next()) {
		String sname = rs.getString(1);
		String scompany = rs.getString(2);
		String stel = rs.getString(3);
		Supplier sr = new Supplier(sname,scompany,stel,company);
		srl.add(sr);
...
<body class="cbp-spmenu-push">
	<div class="main-content">
		...
		<div id="page-wrapper">
			<a href="addsupplier.jsp?company=<%=company%>&email=<%=loginemail%>"
			...
			<table border="1"
				...
				<c:forEach items="${requestScope.srl}" var="mi">
					<tr style="text-align: center;">
						<td>${mi.supname}</td>
						<td>${mi.supunityname}</td>
						<td>${mi.suptel}</td>
